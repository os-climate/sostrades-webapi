...
class TestCalculation(DatabaseUnitTestConfiguration):
    ...
    def test_05_calculation_logs(self):
        from sos_trades_api.controllers.sostrades_data.calculation_controller import calculation_logs
        from sos_trades_api.models.database_models import StudyCaseExecutionLog, StudyCase
        with DatabaseUnitTestConfiguration.app.app_context():
            sc = StudyCase.query.filter(
                StudyCase.name == self.test_study_name).first()
            sc_logs_from_db = StudyCaseExecutionLog.query \
                .filter(StudyCaseExecutionLog.study_case_id == sc.id)\
                .filter(StudyCaseExecutionLog.study_case_execution_id == sc.current_execution_id)\
                .order_by(StudyCaseExecutionLog.id.desc()).limit(300).all()

            sc_logs_from_db.reverse()
            sc_logs_from_method = calculation_logs(
                sc.id, sc.current_execution_id)
            self.assertEqual(sc_logs_from_db, sc_logs_from_method,
                             'Logs from method and from database do not match')
